...
def test_curve_exports_smoke(tmp_path):
    out = tmp_path / 'out'
    run(f"python -m src.main --mode pipeline --config configs/debug.yaml --instance small --seeds 1 --methods uniform battery_threshold am_dueling_ddqn_dr --smoke --output-dir {out} --overwrite --train-if-missing")
