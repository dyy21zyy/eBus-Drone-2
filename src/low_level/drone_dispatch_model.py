...
def parcel_priority_cost(parcel: dict, now: float, eta_l_d: float, eta_u_d: float) -> float:
    t_out = float(parcel.get("T_out_min", parcel.get("T_out", 0.0)))
    deadline = float(parcel.get("delivery_deadline_min", parcel.get("deadline_min", parcel.get("deadline", now))))
    base = float(parcel.get("drone_cost", parcel.get("c_D", 0.0)))
